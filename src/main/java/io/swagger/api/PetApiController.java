...
public class PetApiController implements PetApi {
    ...
    public ResponseEntity<String> petAddGet(@NotNull @ApiParam(value = "", required = true) @Valid @RequestParam(value = "a1", required = true) Integer a1
,@NotNull @ApiParam(value = "", required = true) @Valid @RequestParam(value = "a2", required = true) Integer a2
) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            return new ResponseEntity<String>("adding two numbers, Result is :" + (a1+a2), HttpStatus.OK);
        }
